...
impl SharedMemorySegment {
    ...
    pub fn create<P: AsRef<Path>>(name: P, size: usize) -> ZeroCopyResult<Self> {
        ...
        debug!("Created shared memory segment: {}, size: {} bytes", name, total_size);
    ...
    pub fn open<P: AsRef<Path>>(name: P) -> ZeroCopyResult<Self> {
        ...
        unsafe {
            ...
            if (*header).version != SHARED_MEMORY_VERSION {
                return Err(ZeroCopyError::SharedMemory(
                    format!("Unsupported shared memory version: {}", (*header).version)
                ));
            }
        }
        ...
        debug!("Opened shared memory segment: {}, size: {} bytes", name, size);
    ...
    pub fn writer(&self) -> ZeroCopyResult<SharedMemoryWriter> {
        ...
        unsafe {
            if (*self.header).write_lock.compare_exchange(
                false, 
                true, 
                Ordering::AcqRel, 
                Ordering::Relaxed
...
impl<'a> Drop for SharedMemoryReader<'a> {
    fn drop(&mut self) {
        unsafe {
            (*self.segment.header).readers.fetch_sub(1, Ordering::AcqRel);
        }
    }
}
...
impl<'a> SharedMemoryWriter<'a> {
    ...
    pub fn write(&mut self, data: &[u8]) -> ZeroCopyResult<()> {
        ...
        // Update generation
        unsafe {
            (*self.segment.header).generation.fetch_add(1, Ordering::Release);
        }
...
impl<'a> Drop for SharedMemoryWriter<'a> {
    fn drop(&mut self) {
        unsafe {
            (*self.segment.header).writers.fetch_sub(1, Ordering::AcqRel);
            (*self.segment.header).write_lock.store(false, Ordering::Release);
        }
    }
}
...
impl SharedMemoryManager {
    ...
    pub fn get_or_create(&self, name: &str, size: usize) -> ZeroCopyResult<Arc<SharedMemorySegment>> {
...
impl SharedMemoryLock {
    ...
    pub fn try_lock(&self) -> bool {
        ...
        let lock_ptr = unsafe {
            data.as_ptr().add(self.lock_offset) as *const AtomicBool
        };
        
        unsafe {
            (*lock_ptr).compare_exchange(
                false,
                true,
                Ordering::AcqRel,
                Ordering::Relaxed,
