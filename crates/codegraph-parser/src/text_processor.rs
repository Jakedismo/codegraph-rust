...
impl TextProcessor {
    ...
    pub async fn tokenize_language_aware(&self, content: &str, language: Option<Language>) -> Result<Vec<LanguageToken>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Some(mut parser) = parser_opt.or_else(|| registry.create_parser(&language)) {
                if let Some(tree) = parser.parse(&content, None) {
                    Self::extract_tokens_from_tree_optimized(&tree, &content, &language, &mut tokens, &keyword_cache);
            ...
            Self::extract_tokens_regex_optimized(&content, &language, &mut tokens, &regex_cache, &keyword_cache);
            Ok(tokens)
        }).await
        ...
        while let Some(node) = stack.pop() {
            ...
            if node.child_count() == 0 {
                ...
                if let Ok(node_text) = std::str::from_utf8(&content_bytes[start_byte..end_byte]) {
                    let token_type = Self::classify_token_type_optimized(node.kind(), node_text, language, keyword_cache);
    ...
    fn extract_tokens_from_tree(tree: &Tree, content: &str, language: &Language, tokens: &mut Vec<LanguageToken>) {
        let root = tree.root_node();
        let mut cursor = root.walk();
        let content_bytes = content.as_bytes();
    ...
    fn extract_tokens_regex(content: &str, language: &Language, tokens: &mut Vec<LanguageToken>) {
        ...
        for (i, line) in content.lines().enumerate() {
            ...
            let line_start_byte = content[..content.split('\n').take(i).map(|l| l.len() + 1).sum::<usize>()].len();
    ...
    fn extract_tokens_regex_optimized(
        ...
        for (line_idx, line) in content.lines().enumerate() {
            for (pattern_name, pattern_str, default_type) in &patterns {
                let regex = regex_cache.entry(pattern_name.to_string())
                    .or_insert_with(|| Regex::new(pattern_str).unwrap_or_else(|_| Regex::new(r"\w+").unwrap()));
    ...
    fn classify_token_type(node_kind: &str, content: &str, language: &Language) -> TokenType {
        match node_kind {
            ...
            kind if matches!(kind, "+" | "-" | "*" | "/" | "=" | "==" | "!=" | "<" | ">" | "<=" | ">=" | "&&" | "||" | "!" | "&" | "|" | "^" | "~" | "<<" | ">>" | "+=" | "-=" | "*=" | "/=" | "%=" | "&=" | "|=" | "^=" | "<<=" | ">>=") => TokenType::Operator,
            kind if matches!(kind, "(" | ")" | "[" | "]" | "{" | "}" | ";" | "," | ".") => TokenType::Punctuation,
            _ => {
    ...
    fn get_language_keywords(language: &Language) -> HashSet<&'static str> {
        match language {
            Language::Rust => {
                ["fn", "let", "mut", "const", "static", "if", "else", "match", "for", "while", "loop", "break", "continue", "return", "struct", "enum", "impl", "trait", "mod", "use", "pub", "crate", "super", "self", "Self", "async", "await", "move", "ref", "where", "type", "as", "true", "false", "unsafe", "extern", "dyn"].into_iter().collect()
            },
            Language::Python => {
                ["def", "class", "if", "elif", "else", "for", "while", "try", "except", "finally", "with", "as", "import", "from", "return", "yield", "lambda", "and", "or", "not", "in", "is", "True", "False", "None", "pass", "break", "continue", "global", "nonlocal", "assert", "raise", "del"].into_iter().collect()
            },
            Language::JavaScript | Language::TypeScript => {
                ["function", "var", "let", "const", "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "return", "try", "catch", "finally", "throw", "new", "this", "typeof", "instanceof", "in", "true", "false", "null", "undefined", "class", "extends", "super", "import", "export", "async", "await"].into_iter().collect()
            },
            Language::Go => {
                ["func", "var", "const", "if", "else", "for", "switch", "case", "default", "break", "continue", "return", "go", "defer", "chan", "select", "type", "struct", "interface", "map", "package", "import", "true", "false", "nil", "range"].into_iter().collect()
            },
            Language::Java => {
                ["public", "private", "protected", "static", "final", "abstract", "class", "interface", "extends", "implements", "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "return", "try", "catch", "finally", "throw", "throws", "new", "this", "super", "import", "package", "true", "false", "null"].into_iter().collect()
            },
            Language::Cpp => {
                ["auto", "bool", "char", "const", "double", "float", "int", "long", "short", "signed", "unsigned", "void", "class", "struct", "public", "private", "protected", "virtual", "static", "extern", "inline", "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "return", "try", "catch", "throw", "new", "delete", "this", "true", "false", "nullptr", "template", "typename", "namespace", "using"].into_iter().collect()
            },
            _ => HashSet::new(),
        }
    }
    ...
    pub async fn chunk_semantic(&self, content: &str, language: Option<Language>) -> Result<Vec<TextChunk>> {
        let language = language.unwrap_or(Language::Other("text".to_string()));
        let registry = self.language_registry.clone();
        let content = content.to_string();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Some(mut parser) = registry.create_parser(&language) {
                if let Some(tree) = parser.parse(&content, None) {
                    Self::extract_semantic_chunks_from_tree(&tree, &content, &language, &config, &mut chunks);
                    return Ok(chunks);
                }
            }
    ...
    fn extract_semantic_chunks_from_tree(
        ...
        loop {
            ...
            if node.is_error() || Self::is_range_processed(&processed_ranges, node.start_byte(), node.end_byte()) {
                if !cursor.goto_next_sibling() {
                    if !cursor.goto_parent() {
                        break;
                    }
            ...
            if Self::is_semantic_boundary(&node, language) {
                ...
                if chunk_size >= config.min_chunk_size && chunk_size <= config.max_chunk_size {
                    if let Ok(chunk_content) = std::str::from_utf8(&content_bytes[start_byte..end_byte]) {
                        ...
                        let (context_before, context_after) = if config.preserve_semantic_boundaries {
                            Self::extract_surrounding_context(content, start_byte, end_byte, &config)
                        } else {
                            (None, None)
                        };
                    ...
                    Self::split_large_chunk(&node, content, language, config, chunks, &mut processed_ranges);
                }
            }
    ...
    fn extract_chunks_text_based(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if potential_size > config.max_chunk_size && current_chunk.len() >= config.min_chunk_size {
        ...
        match language {
            ...
            Language::Go => {
                matches!(node.kind(), 
                    ...
                    "var_declaration" | "const_declaration" | "package_clause" | "import_declaration"
                )
            },
            Language::Java => {
                matches!(node.kind(), 
            ...
            Language::Cpp => {
                matches!(node.kind(), 
                    ...
                    "namespace_definition" | "preproc_include" | "declaration"
                )
            },
            _ => {
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if Self::is_semantic_boundary(&child, language) {
                    ...
                    if chunk_size >= config.min_chunk_size && chunk_size <= config.max_chunk_size {
                        if let Ok(chunk_content) = std::str::from_utf8(&content.as_bytes()[start_byte..end_byte]) {
    ...
    fn is_range_processed(processed_ranges: &[(usize, usize)], start: usize, end: usize) -> bool {
        processed_ranges.iter().any(|(s, e)| start >= *s && end <= *e)
    }
    ...
    fn apply_overlap(chunks: &mut Vec<TextChunk>, content: &str, overlap_size: usize) {
        ...
        for i in 1..chunks.len() {
            ...
            if curr_start > prev_end && curr_start - prev_end > overlap_size {
                ...
                if let Ok(overlap_content) = std::str::from_utf8(&content.as_bytes()[prev_end..prev_end + overlap_size]) {
                    chunks[i - 1].content.push_str("\n...\n");
                    chunks[i - 1].content.push_str(overlap_content.trim());
                }

                // Add overlap to current chunk's beginning
                if let Ok(overlap_content) = std::str::from_utf8(&content.as_bytes()[curr_start - overlap_size..curr_start]) {
    ...
    pub async fn extract_context(&self, chunk: &TextChunk, full_content: &str) -> Result<ContextExtraction> {
        let language = chunk.language.clone().unwrap_or(Language::Other("text".to_string()));
        let registry = self.language_registry.clone();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let importance_score = Self::calculate_importance_score(&chunk_content, &relationships, &language);
    ...
    fn find_function_calls(
        ...
        loop {
            ...
            let is_call = match language {
                ...
                Language::JavaScript | Language::TypeScript => matches!(current.kind(), "call_expression"),
            ...
            if is_call {
                if let Ok(call_text) = std::str::from_utf8(&content_bytes[current.start_byte()..current.end_byte()]) {
    ...
    fn find_variable_relationships(
        ...
        loop {
            ...
            let is_identifier = match language {
                ...
                Language::JavaScript | Language::TypeScript => matches!(current.kind(), "identifier"),
            ...
            if is_identifier {
                if let Ok(id_text) = std::str::from_utf8(&content_bytes[current.start_byte()..current.end_byte()]) {
                    // Determine if this is a definition or use based on parent context
                    if let Some(parent) = current.parent() {
                        let relation_type = match parent.kind() {
                            kind if kind.contains("declaration") || kind.contains("definition") => RelationType::Defines,
                            _ => RelationType::Uses,
                        };
    ...
    fn find_type_relationships(
        ...
        loop {
            ...
            let is_type_ref = match language {
                ...
                Language::JavaScript | Language::TypeScript => matches!(current.kind(), "type_identifier"),
            ...
            if is_type_ref {
                if let Ok(type_text) = std::str::from_utf8(&content_bytes[current.start_byte()..current.end_byte()]) {
                    relationships.push(SemanticRelationship {
    ...
    pub async fn deduplicate_and_normalize(&self, chunks: Vec<TextChunk>) -> Result<Vec<TextChunk>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for mut chunk in chunks {
                ...
                chunk.content = Self::normalize_content(&chunk.content, &chunk.language, &config.normalization_level);
    ...
    fn normalize_content(content: &str, language: &Option<Language>, level: &NormalizationLevel) -> String {
        match level {
            ...
            NormalizationLevel::Standard => {
                let mut normalized = Self::normalize_content(content, language, &NormalizationLevel::Basic);
            ...
            NormalizationLevel::Aggressive => {
                let mut normalized = Self::normalize_content(content, language, &NormalizationLevel::Standard);
    ...
    fn remove_comments(content: &str, language: &Option<Language>) -> String {
        if let Some(lang) = language {
            match lang {
                Language::Rust | Language::Cpp | Language::JavaScript | Language::TypeScript | Language::Go | Language::Java => {
    ...
    fn initialize_normalization_patterns(&self) {
        ...
        for language in &[Language::Rust, Language::Python, Language::JavaScript, Language::TypeScript, Language::Go, Language::Java, Language::Cpp] {
            ...
            self.normalization_patterns.insert(language.clone(), patterns);
        }
    }
    ...
    fn initialize_keyword_cache(&self) {
        ...
        self.keyword_cache.insert(Language::JavaScript, js_keywords.clone());
...
mod tests {
    ...
    async fn test_context_extraction() {
        ...
        let full_content = "impl User { fn new(name: String, age: u32) -> Self { Self { name, age } } }";
    ...
    async fn test_configuration_options() {
        ...
        assert_eq!(processor.config.normalization_level, NormalizationLevel::Aggressive);
    }
