...
impl<T> WaitFreeSpscQueue<T> {
    ...
    #[inline]
    fn slot(&self, idx: usize) -> &UnsafeCell<MaybeUninit<T>> { &self.buf[idx & self.mask] }
}

/// Producer side of the SPSC queue.
pub struct Producer<T> { pub(crate) inner: *mut WaitFreeSpscQueue<T> }

/// Consumer side of the SPSC queue.
pub struct Consumer<T> { pub(crate) inner: *mut WaitFreeSpscQueue<T> }
...
impl<T> Producer<T> {
    ...
    pub fn try_push(&self, value: T) -> Result<(), SpscError> {
        ...
        if tail.wrapping_sub(head) == q.capacity() - 1 { // full: keep one slot open
...
impl<T> Consumer<T> {
    ...
    pub fn try_pop(&self) -> Result<T, SpscError> {
        ...
        if head == tail { return Err(SpscError::Empty); }
    ...
    fn spsc_concurrent() {
        ...
        let t = thread::spawn(move || {
            for i in 0..10_000u32 {
                loop { if p.try_push(i).is_ok() { break; } }
            }
        });
        let mut count = 0u32;
        while count < 10_000 {
    ...
    mod loom_tests {
        ...
        fn loom_spsc() {
            loom::model(|| {
                ...
                let tp = thread::spawn(move || { p.try_push(1).ok(); });
                let tc = thread::spawn(move || { let _ = c.try_pop(); });
                tp.join().unwrap();
                tc.join().unwrap();
            });
        }
