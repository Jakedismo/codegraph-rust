...
impl Balancer for SmoothWeightedRR {
    fn name(&self) -> &'static str { "smooth_wrr" }
    ...
    fn pick(&self, pool: &EndpointPool, _key: Option<&[u8]>) -> Option<Arc<Endpoint>> {
        ...
        if st.weights.len() != eps.len() {
            st.weights = eps.iter().map(|e| e.weight.load(std::sync::atomic::Ordering::Relaxed) as i64).collect();
        ...
        for i in 0..eps.len() {
            ...
            if st.current[i] > st.current[best] { best = i; }
        }
