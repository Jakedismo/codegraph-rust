...
impl LeastConnections { pub fn new() -> Self { Self } }
impl Default for LeastConnections { fn default() -> Self { Self::new() } }

impl Balancer for LeastConnections {
    fn name(&self) -> &'static str { "least_conn" }
    ...
    fn pick(&self, pool: &EndpointPool, _key: Option<&[u8]>) -> Option<Arc<Endpoint>> {
        ...
        for ep in eps {
            let c = ep.open_connections.load(std::sync::atomic::Ordering::Relaxed);
            if c < best_conn {
                best_conn = c;
                best = Some(ep);
            }
