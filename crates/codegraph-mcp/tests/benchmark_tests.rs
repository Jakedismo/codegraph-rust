...
async fn benchmark_message_serialization() {
    ...
    println!("Running message serialization benchmark with {} iterations", iterations);
    ...
    println!("Average message serialization/deserialization time: {}μs", avg_duration.as_micros());
    
    // Assert that average time is well under our 50ms target (should be microseconds, not milliseconds)
    assert!(avg_duration.as_millis() < 1, "Message processing should be under 1ms, got {}μs", avg_duration.as_micros());
}
...
async fn benchmark_protocol_operations() {
    ...
    println!("Running protocol operations benchmark with {} iterations", iterations);
    ...
    for i in 0..iterations {
        ...
        // Build notification  
        let _notification = protocol.build_notification("test_notification", &params).unwrap();
    }
    
    let total_duration = start.elapsed();
    let avg_duration = total_duration / iterations as u32;
    
    println!("Average protocol operation time: {}μs", avg_duration.as_micros());
    
    // Should be very fast - well under 1ms per operation
    assert!(avg_duration.as_millis() < 1, "Protocol operations should be under 1ms, got {}μs", avg_duration.as_micros());
}
...
async fn benchmark_version_negotiation() {
    ...
    println!("Running version negotiation benchmark with {} iterations", iterations);
    
    let start = Instant::now();
    
    for i in 0..iterations {
        let version = if i % 2 == 0 { "2025-03-26" } else { "2024-11-05" };
    ...
    println!("Average version negotiation time: {}ns", avg_duration.as_nanos());
    
    // Version negotiation should be extremely fast - nanoseconds
    assert!(avg_duration.as_micros() < 100, "Version negotiation should be under 100μs, got {}ns", avg_duration.as_nanos());
}
...
fn benchmark_heartbeat_creation() {
    ...
    println!("Running heartbeat creation benchmark with {} iterations", iterations);
    ...
    println!("Average heartbeat manager creation time: {}μs", avg_duration.as_micros());
    
    // Heartbeat creation should be fast
    assert!(avg_duration.as_millis() < 1, "Heartbeat creation should be under 1ms, got {}μs", avg_duration.as_micros());
}
...
fn benchmark_error_creation() {
    ...
    println!("Running error creation benchmark with {} iterations", iterations);
    ...
    assert!(avg_duration.as_micros() < 10, "Error creation should be under 10μs, got {}ns", avg_duration.as_nanos());
}
...
async fn performance_system_responsiveness() {
    ...
    let request = protocol.build_request("complex_operation", &params).unwrap();
    let message = JsonRpcMessage::V2(JsonRpcV2Message::Request(request));
    let serialized = serde_json::to_string(&message).unwrap();
    let _deserialized: JsonRpcMessage = serde_json::from_str(&serialized).unwrap();
    ...
    println!("Total system responsiveness test duration: {}μs", total_duration.as_micros());
    
    // Entire sequence should complete in well under 1ms
    assert!(total_duration.as_millis() < 1, "System responsiveness test should complete under 1ms, got {}μs", total_duration.as_micros());
}