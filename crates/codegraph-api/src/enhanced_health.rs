...
async fn check_parser_health_enhanced(state: &AppState) -> ComponentStatus {
    ...
    ComponentStatus {
        status: if all_passed { "healthy".to_string() } else { "degraded".to_string() },
        last_check: current_timestamp(),
        response_time_ms: Some(response_time),
        details: Some(details),
        error: if all_passed { None } else { Some("Some parser tests failed".to_string()) },
        health_score: Some(health_score),
    }
}
...
async fn check_memory_health_enhanced() -> ComponentStatus {
    #[cfg(feature = "leak-detect")]
    {
        ...
        match tracker.get_stats() {
            Ok(stats) => {
                ...
                let health_score = if leak_ratio < 0.01 { 1.0 }
                                 else if leak_ratio < 0.05 { 0.8 }
                                 else if leak_ratio < 0.1 { 0.6 }
                                 else { 0.2 };
                
                let status = if stats.leaked_memory > 50 * 1024 * 1024 { // 50MB
                    "unhealthy".to_string()
                } else if stats.leaked_memory > 10 * 1024 * 1024 { // 10MB
                    "degraded".to_string()
                } else {
                    "healthy".to_string()
                };
                ...
                ComponentStatus {
                    ...
                    error: if stats.leaked_memory > 10 * 1024 * 1024 {
                        Some(format!("Memory leaks detected: {}MB", stats.leaked_memory / 1024 / 1024))
                    } else { None },
                    health_score: Some(health_score),
                }
            }
    ...
    #[cfg(not(feature = "leak-detect"))]
    {
        ...
        let health_score = if memory_usage_percent < 80.0 { 1.0 }
                          else if memory_usage_percent < 90.0 { 0.7 }
                          else if memory_usage_percent < 95.0 { 0.4 }
                          else { 0.1 };
        
        ComponentStatus {
            status: if memory_usage_percent > 95.0 { "unhealthy".to_string() }
                   else if memory_usage_percent > 85.0 { "degraded".to_string() }
...
async fn check_storage_health_enhanced(state: &AppState) -> ComponentStatus {
    ...
    let health_score = if write_test && read_test && disk_usage < 90.0 { 1.0 }
                      else if write_test && read_test && disk_usage < 95.0 { 0.7 }
                      else if write_test && read_test { 0.5 }
                      else { 0.0 };
    
    ComponentStatus {
        status: if health_score > 0.8 { "healthy".to_string() }
               else if health_score > 0.4 { "degraded".to_string() }
               else { "unhealthy".to_string() },
        last_check: current_timestamp(),
        response_time_ms: Some(0),
        details: Some(details),
...
async fn check_connection_pool_health(state: &AppState) -> ComponentStatus {
    ...
    details.insert("utilization_percent".to_string(), format!("{:.1}", utilization * 100.0));
    
    let health_score = if utilization < 0.8 { 1.0 }
                      else if utilization < 0.9 { 0.7 }
                      else if utilization < 0.95 { 0.4 }
                      else { 0.1 };
    ...
    ComponentStatus {
        ...
        details: Some(details),
        error: if health_score < 0.5 {
            Some(format!("High connection pool utilization: {:.1}%", utilization * 100.0))
        } else { None },
        health_score: Some(health_score),
    }
}
...
async fn update_prometheus_health_metrics(
    ...
    record_health_check("database", components.database.status == "healthy", 
                       components.database.response_time_ms.unwrap_or(0) as f64 / 1000.0);
    record_health_check("vector_search", components.vector_search.status == "healthy",
                       components.vector_search.response_time_ms.unwrap_or(0) as f64 / 1000.0);
    record_health_check("parser", components.parser.status == "healthy",
    ...
    record_health_check("storage", components.storage.status == "healthy",
                       components.storage.response_time_ms.unwrap_or(0) as f64 / 1000.0);
    record_health_check("connection_pool", components.connection_pool.status == "healthy", 0.0);
    ...
    for (component, status) in [
        ...
        if status.status == "unhealthy" {
            alerts.push(AlertInfo {
                ...
                message: status.error.clone().unwrap_or_else(|| "Component unhealthy".to_string()),
                threshold: None,
            ...
            alerts.push(AlertInfo {
                ...
                component: component.to_string(),
                message: status.error.clone().unwrap_or_else(|| "Component degraded".to_string()),
                threshold: None,
                current_value: status.health_score,
                first_seen: current_time,
            });
...
fn determine_overall_status(components: &ComponentsHealth, performance: &PerformanceMetrics) -> String {
...
fn calculate_health_score(response_time_ms: u64, _details: Option<&HashMap<String, String>>) -> f64 {
    // Simple health score based on response time
    if response_time_ms < 100 { 1.0 }
