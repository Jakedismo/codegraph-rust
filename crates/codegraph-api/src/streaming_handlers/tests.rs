...
mod tests {
    ...
    async fn test_large_dataset_stream_performance() {
        ...
        let items: Result<Vec<_>, _> = timeout(timeout_duration, stream.take(500).collect()).await;
    ...
    async fn test_csv_stream_format() {
        let mock_results = vec![codegraph_vector::SearchResult {
            node_id: uuid::Uuid::new_v4(),
            score: 0.9,
        }];
    ...
    fn create_mock_graph_guard(
    ) -> tokio::sync::RwLockReadGuard<'static, Box<dyn codegraph_core::GraphStore + Send + Sync>>
    {
        ...
        static MOCK_GRAPH: std::sync::OnceLock<
            Arc<RwLock<Box<dyn codegraph_core::GraphStore + Send + Sync>>>,
        > = std::sync::OnceLock::new();

        let graph = MOCK_GRAPH.get_or_init(|| {
            Arc::new(RwLock::new(
                Box::new(MockGraphStore) as Box<dyn codegraph_core::GraphStore + Send + Sync>
            ))
        });

        // This is unsafe in real code - using for test simplification
        unsafe { std::mem::transmute(graph.blocking_read()) }
    }
    ...
    impl codegraph_core::GraphStore for MockGraphStore {
        async fn add_node(
            &mut self,
            _node: codegraph_core::Node,
        ) -> Result<(), codegraph_core::CodeGraphError> {
            Ok(())
        }

        async fn get_node(
            &self,
            _id: codegraph_core::NodeId,
        ) -> Result<Option<codegraph_core::Node>, codegraph_core::CodeGraphError> {
            Ok(None)
        }
        ...
        async fn delete_node(
            &mut self,
            _id: codegraph_core::NodeId,
        ) -> Result<bool, codegraph_core::CodeGraphError> {
            Ok(false)
        }
        ...
        async fn get_nodes_by_file(
            &self,
            _file_path: &str,
        ) -> Result<Vec<codegraph_core::Node>, codegraph_core::CodeGraphError> {
            Ok(vec![])
        }
...
mod integration_tests {
    ...
    async fn test_stream_metadata_endpoint() {
        ...
        let response = server.get("/stream/test-stream-123/metadata").await;
    ...
    async fn test_flow_control_stats_endpoint() {
        ...
        let response = server.get("/stream/stats").await;
    ...
    async fn create_test_router() -> Router {
        ...
        Router::new()
            ...
            .route(
                "/stream/:id/metadata",
                axum::routing::get(get_stream_metadata),
            )
