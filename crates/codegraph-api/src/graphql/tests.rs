...
async fn test_performance_within_targets() {
    ...
    assert!(res.errors.is_empty());
    assert!(elapsed.as_millis() < 50, "Simple query took {}ms, should be <50ms", elapsed.as_millis());
    ...
    assert!(res.errors.is_empty());
    assert!(elapsed.as_millis() < 200, "Complex query took {}ms, should be <200ms", elapsed.as_millis());
}
...
async fn test_dataloader_batching() {
    ...
    assert!(elapsed.as_millis() < 100, "Batched queries took {}ms, batching may not be working", elapsed.as_millis());
...
async fn test_query_complexity_limits() {
    ...
    if !res.errors.is_empty() {
        ...
        assert!(has_complexity_error || res.errors.iter().any(|e| 
            !e.message.to_lowercase().contains("complexity")));
    }
}
...
mod integration_tests {
    ...
    async fn test_full_graphql_pipeline() {
        ...
        assert!(res.errors.is_empty(), "Workflow query failed: {:?}", res.errors);
        ...
        assert!(elapsed.as_millis() < 500, "Full workflow took {}ms", elapsed.as_millis());
    }
}