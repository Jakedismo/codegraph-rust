use codegraph_cache::{CacheEntriesSoA, CacheOptimizedHashMap, PaddedAtomicUsize};
...
fn bench_structure_of_arrays(c: &mut Criterion) {
    ...
    for size in CACHE_SIZES {
        ...
        group.bench_with_input(BenchmarkId::new("sequential_scan", size), size, |b, _| {
            let mut soa = CacheEntriesSoA::new(*size);
            for (key, value) in &data {
                soa.insert(key.clone(), *value, 8);
            }
            ...
            |b, &threads| {
                b.iter(|| {
                    let counters: Vec<Arc<Mutex<usize>>> =
                        (0..threads).map(|_| Arc::new(Mutex::new(0))).collect();
                    ...
                    let sum: usize = counters.iter().map(|c| *c.lock().unwrap()).sum();
                    black_box(sum);
                });
            },
        );
