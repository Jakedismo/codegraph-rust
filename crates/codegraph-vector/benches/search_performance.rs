use codegraph_vector::{
    BatchConfig, BatchProcessor, FaissIndexManager, IndexConfig, IndexType, OptimizedSearchEngine,
    SearchConfig,
};
...
fn generate_random_vectors(count: usize, dimension: usize) -> Vec<Vec<f32>> {
    ...
    (0..count)
        .map(|_| (0..dimension).map(|_| rng.f32() - 0.5).collect())
        .collect()
}
            ...
            |b, index_type| {
                ...
                b.to_async(&rt).iter(|| async {
                    ...
                    let flat_vectors: Vec<f32> =
                        vectors.iter().flat_map(|v| v.iter().cloned()).collect();
...
fn bench_optimized_search(c: &mut Criterion) {
    ...
    c.bench_function("optimized_search_cold", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let _results = black_box(
                search_engine
                    .batch_search_knn(&query_refs[0..10], k)
                    .await
                    .unwrap(),
            );
        });
    ...
    rt.block_on(async {
        let query_refs: Vec<&[f32]> = queries.iter().map(|v| v.as_slice()).collect();
        let _ = search_engine
            .batch_search_knn(&query_refs[0..100], k)
            .await
            .unwrap();
    });
        ...
        group.bench_with_input(
            ...
            |b, &batch_size| {
                b.to_async(&rt).iter(|| async {
                    ...
                    let mut processor =
                        BatchProcessor::new(batch_config, index_config, None).unwrap();
...
fn bench_parallel_vs_sequential(c: &mut Criterion) {
    ...
    let setup_engine = |parallel: bool| {
        let config = if parallel {
            search_config_parallel.clone()
        } else {
            search_config_sequential.clone()
        };
