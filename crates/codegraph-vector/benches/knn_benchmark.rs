...
fn create_dataset(size: usize, dimension: usize) -> Vec<CodeNode> {
    ...
    let languages = [Language::Rust, Language::Python, Language::JavaScript, Language::Go];
    let node_types = [NodeType::Function, NodeType::Struct, NodeType::Class, NodeType::Module];
        ...
        c.bench_with_input(
            ...
            |b, _| {
                b.to_async(&rt).iter(|| async {
                    let result = engine.single_similarity_search(
...
fn benchmark_parallel_search(c: &mut Criterion) {
    ...
    for &max_parallel in &parallelism_levels {
        ...
        c.bench_with_input(
            ...
            |b, _| {
                b.to_async(&rt).iter(|| async {
                    let result = engine.parallel_similarity_search(
...
fn benchmark_clustering_impact(c: &mut Criterion) {
    ...
    for clustering_enabled in [false, true] {
        ...
        let query_embedding = generate_random_embedding(dimension, 999);
        let bench_name = if clustering_enabled { "with_clustering" } else { "without_clustering" };
...
fn benchmark_cache_performance(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for (i, embedding) in query_embeddings.iter().enumerate().take(50) {
            if i % 10 == 0 {
                let _ = engine.single_similarity_search(
                    embedding.clone(),
                    SearchConfig::default(),
                ).await.unwrap();
            }
...
fn benchmark_batch_operations(c: &mut Criterion) {
    ...
    for &batch_size in &batch_sizes {
        ...
        c.bench_with_input(
            ...
            |b, _| {
                b.to_async(&rt).iter(|| async {
                    let result = engine.batch_search_similar_functions(
                        black_box(&function_nodes),
                        None,
                    ).await.unwrap();
                    black_box(result);
                });
...
fn benchmark_index_building(c: &mut Criterion) {
    ...
    for &size in &dataset_sizes {
        ...
        c.bench_with_input(
            BenchmarkId::new("index_building", size),
            &size,
            |b, _| {
                b.to_async(&rt).iter(|| async {
                    let engine = OptimizedKnnEngine::new(dimension, SearchConfig::default()).unwrap();
