...
impl OnnxEmbeddingProvider {
    pub async fn new(config: OnnxConfig) -> Result<Self> {
        ...
        let (tokenizer_path, model_path) = if Path::new(&config.model_repo).exists() {
            ...
            let candidates: Vec<String> = if let Some(mf) = &config.model_file {
                vec![mf.clone(), "onnx/model.onnx".into(), "model.onnx".into(), "model_fp16.onnx".into()]
            } else {
                vec!["model.onnx".into(), "onnx/model.onnx".into(), "model_fp16.onnx".into()]
            };
            let mut found = None;
    ...
    fn prepare_inputs(&self, texts: &[String]) -> Result<(Array2<i64>, Array2<i64>, Array2<i64>, usize)> {
        ...
        for t in texts {
            ...
            ids.push(tid); mask.push(m); type_ids.push(tt);
        }
        ...
        for i in 0..b {
            ...
            arr_type_ids.slice_mut(s![i, ..]).assign(&ndarray::ArrayView1::from(tt.as_slice()));
        }
        ...
        for chunk in texts.chunks(config.batch_size.max(1)) {
            ...
            let arr: Array2<f32> = {
                ...
                for inp in &sess.inputs {
                    ...
                    if n.contains("input_ids") || n == "input" { // some models use generic name
                ...
                if arr_dyn.ndim() == 2 {
                    ...
                    let counts = mask_f.sum_axis(Axis(1)).mapv(|x| if x <= 0.0 { 1.0 } else { x }); // [B]
                    let mut pooled = Array2::<f32>::zeros((b, h));
                    for i in 0..b {
                        let denom = counts[i];
                        pooled.slice_mut(s![i, ..]).assign(&(&sum_emb.slice(s![i, ..]) / denom));
                    }
                    pooled
                } else {
                    return Err(CodeGraphError::External("Unexpected ONNX output rank; expected 2D or 3D tensor".into()));
                }
            };
