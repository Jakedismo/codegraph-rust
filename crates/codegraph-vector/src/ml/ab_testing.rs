...
impl ABTestingFramework {
    ...
    pub async fn start_experiment(&self, experiment_id: &str) -> Result<()> {
        ...
        let experiment = experiments.get_mut(experiment_id).ok_or_else(|| {
            CodeGraphError::NotFound(format!("Experiment {} not found", experiment_id))
        })?;
    ...
    pub async fn assign_variant(&self, experiment_id: &str, session_id: &str) -> Result<String> {
        ...
        let experiment = experiments.get(experiment_id).ok_or_else(|| {
            CodeGraphError::NotFound(format!("Experiment {} not found", experiment_id))
        })?;
        ...
        if experiment.config.traffic_allocation.sticky_sessions {
            ...
            let experiment_assignments = assignments
    ...
    pub async fn run_inference_with_test(
        ...
        let engine = engines.get(&variant).ok_or_else(|| {
            CodeGraphError::NotFound(format!("Engine for variant {} not found", variant))
        })?;
    ...
    pub async fn analyze_experiment(&self, experiment_id: &str) -> Result<ExperimentResults> {
        ...
        for data_point in &experiment_data {
            variant_data
                .entry(data_point.variant.clone())
                .or_default()
                .push(data_point);
        }
        ...
        match allocation.strategy {
            AllocationStrategy::Random => {
                ...
                allocation
                    ...
                    .cloned()
                    .ok_or_else(|| {
                        CodeGraphError::Configuration("No variants configured".to_string())
                    })
            }
    ...
    async fn calculate_variant_results(
        &self,
        variant_name: &str,
    ...
    fn calculate_metric_summary(&self, name: &str, values: &[f64]) -> MetricSummary {
        ...
        let p95 = sorted_values
            .get(p95_idx)
            .copied()
            .unwrap_or(sorted_values.last().copied().unwrap_or(0.0));
        let p99 = sorted_values
            .get(p99_idx)
            .copied()
            .unwrap_or(sorted_values.last().copied().unwrap_or(0.0));
    ...
    async fn perform_statistical_comparisons(
        &self,
        variant_data: &HashMap<String, Vec<&ExperimentDataPoint>>,
        ...
        for i in 0..variants.len() {
            for j in (i + 1)..variants.len() {
                ...
                let baseline_latencies: Vec<f64> = variant_data[baseline]
                    .iter()
                    .filter_map(|dp| dp.metrics.get("latency_ms"))
                    .copied()
                    .collect();

                let treatment_latencies: Vec<f64> = variant_data[treatment]
                    .iter()
                    .filter_map(|dp| dp.metrics.get("latency_ms"))
                    .copied()
                    .collect();
                ...
                if !baseline_latencies.is_empty() && !treatment_latencies.is_empty() {
                    let comparison = self.perform_t_test(
                        &baseline_latencies,
                        &treatment_latencies,
                        baseline,
                        treatment,
    ...
    fn perform_t_test(
        ...
        let treatment_var = treatment
            .iter()
            .map(|x| (x - treatment_mean).powi(2))
            .sum::<f64>()
            / (treatment.len() - 1) as f64;

        let pooled_var = ((baseline.len() - 1) as f64 * baseline_var
            + (treatment.len() - 1) as f64 * treatment_var)
            / (baseline.len() + treatment.len() - 2) as f64;

        let std_err =
            (pooled_var * (1.0 / baseline.len() as f64 + 1.0 / treatment.len() as f64)).sqrt();
    ...
    fn determine_winner(&self, comparisons: &[VariantComparison]) -> Option<String> {
        ...
        for comparison in comparisons {
            if comparison.significant && comparison.effect_size < 0.0 {
    ...
    fn generate_recommendations(
        ...
        if recommendations.is_empty() {
            recommendations
                .push("No clear winner identified. Consider collecting more data.".to_string());
        }
...
mod tests {
    ...
    async fn test_variant_assignment() {
        ...
        let variant1 = framework
            .assign_variant(&experiment_id, "user1")
            .await
            .unwrap();
