...
impl PersistentStorage {
    ...
    pub fn new(base_path: PathBuf) -> Result<Self> {
        std::fs::create_dir_all(&base_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to create storage directory: {}", e)))?;
    ...
    fn load_metadata(path: &Path) -> Result<StorageMetadata> {
        let file = File::open(path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to open metadata file: {}", e)))?;
        
        let reader = BufReader::new(file);
        let metadata: StorageMetadata = serde_json::from_reader(reader)
            .map_err(|e| CodeGraphError::Serialization(format!("Failed to deserialize metadata: {}", e)))?;
    ...
    fn save_metadata(&self) -> Result<()> {
        ...
        let file = OpenOptions::new()
            ...
            .truncate(true)
            .open(&metadata_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to create metadata file: {}", e)))?;

        let writer = BufWriter::new(file);
        let metadata = self.metadata.read();
        
        serde_json::to_writer_pretty(writer, &*metadata)
            .map_err(|e| CodeGraphError::Serialization(format!("Failed to serialize metadata: {}", e)))?;
    ...
    pub fn save_index(&self, index: &IndexImpl, config: &IndexConfig) -> Result<()> {
        ...
        if config.compression_level > 0 {
            ...
            index.write(&index_path.to_string_lossy())
    ...
    fn save_compressed_index(&self, index: &IndexImpl, path: &Path, compression_level: u32) -> Result<()> {
        // First save to temporary uncompressed file
        let temp_path = path.with_extension("tmp");
        index.write(&temp_path.to_string_lossy())
            .map_err(|e| CodeGraphError::Vector(format!("Failed to save temporary index: {}", e)))?;

        // Read and compress
        let input_file = File::open(&temp_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to open temporary file: {}", e)))?;
        
        let output_file = File::create(path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to create compressed file: {}", e)))?;
        ...
        std::io::copy(&mut input_reader, &mut encoder)
            .map_err(|e| CodeGraphError::Io(format!("Failed to compress index: {}", e)))?;
        
        encoder.finish()
            .map_err(|e| CodeGraphError::Io(format!("Failed to finalize compression: {}", e)))?;

        // Remove temporary file
        std::fs::remove_file(&temp_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to remove temporary file: {}", e)))?;
    ...
    pub fn load_index(&self, config: &IndexConfig) -> Result<IndexImpl> {
        ...
        let index = if metadata.compression_enabled {
            ...
            IndexImpl::read(&index_path.to_string_lossy())
                .map_err(|e| CodeGraphError::Vector(format!("Failed to load index: {}", e)))?
        };

        info!("Loaded FAISS index from {:?} ({} vectors)", index_path, index.ntotal());
        Ok(index)
    }
    ...
    fn load_compressed_index(&self, path: &Path) -> Result<IndexImpl> {
        let input_file = File::open(path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to open compressed file: {}", e)))?;

        let temp_path = path.with_extension("tmp_decomp");
        let output_file = File::create(&temp_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to create temporary file: {}", e)))?;

        let mut decoder = GzDecoder::new(input_file);
        let mut output_writer = BufWriter::new(output_file);

        std::io::copy(&mut decoder, &mut output_writer)
            .map_err(|e| CodeGraphError::Io(format!("Failed to decompress index: {}", e)))?;

        drop(output_writer); // Ensure file is closed

        let index = IndexImpl::read(&temp_path.to_string_lossy())
            .map_err(|e| CodeGraphError::Vector(format!("Failed to load decompressed index: {}", e)))?;
    ...
    pub fn save_embeddings(&self, embeddings: &HashMap<NodeId, Vec<f32>>) -> Result<()> {
        ...
        let serialized = bincode::serialize(embeddings)
            .map_err(|e| CodeGraphError::Serialization(format!("Failed to serialize embeddings: {}", e)))?;

        std::fs::write(&embeddings_path, &serialized)
            .map_err(|e| CodeGraphError::Io(format!("Failed to write embeddings: {}", e)))?;

        // Create memory map
        let file = OpenOptions::new()
            .read(true)
            .open(&embeddings_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to open embeddings file: {}", e)))?;

        let mmap = unsafe {
            MmapOptions::new()
                .map(&file)
                .map_err(|e| CodeGraphError::Io(format!("Failed to create memory map: {}", e)))?
        };

        *self.embeddings_mmap.write() = Some(mmap);
        
        info!("Saved {} embeddings to memory-mapped file", embeddings.len());
        Ok(())
    }
    ...
    pub fn load_embeddings(&self) -> Result<HashMap<NodeId, Vec<f32>>> {
        ...
        let embeddings: HashMap<NodeId, Vec<f32>> = bincode::deserialize(&mmap[..])
            .map_err(|e| CodeGraphError::Serialization(format!("Failed to deserialize embeddings: {}", e)))?;

        info!("Loaded {} embeddings from memory-mapped file", embeddings.len());
        Ok(embeddings)
    }

    /// Save ID mapping to memory-mapped file
    pub fn save_id_mapping(&self, id_mapping: &HashMap<i64, NodeId>, reverse_mapping: &HashMap<NodeId, i64>) -> Result<()> {
        ...
        let serialized = bincode::serialize(&mapping_data)
            .map_err(|e| CodeGraphError::Serialization(format!("Failed to serialize ID mapping: {}", e)))?;

        std::fs::write(&id_mapping_path, &serialized)
            .map_err(|e| CodeGraphError::Io(format!("Failed to write ID mapping: {}", e)))?;

        // Create memory map
        let file = OpenOptions::new()
            .read(true)
            .open(&id_mapping_path)
            .map_err(|e| CodeGraphError::Io(format!("Failed to open ID mapping file: {}", e)))?;
        ...
        *self.id_mapping_mmap.write() = Some(mmap);
        ...
        info!("Saved ID mappings to memory-mapped file ({} entries)", id_mapping.len());
        Ok(())
    }
    ...
    pub fn load_id_mapping(&self) -> Result<(HashMap<i64, NodeId>, HashMap<NodeId, i64>)> {
        ...
        if self.id_mapping_mmap.read().is_none() {
            let file = File::open(&id_mapping_path)
                .map_err(|e| CodeGraphError::Io(format!("Failed to open ID mapping file: {}", e)))?;
        ...
        let (id_mapping, reverse_mapping): (HashMap<i64, NodeId>, HashMap<NodeId, i64>) = 
            bincode::deserialize(&mmap[..])
                .map_err(|e| CodeGraphError::Serialization(format!("Failed to deserialize ID mapping: {}", e)))?;

        info!("Loaded ID mappings from memory-mapped file ({} entries)", id_mapping.len());
