...
impl QueryProcessor {
    pub fn new() -> Self {
        ...
        let programming_keywords = [
            "function", "fn", "async", "await", "class", "struct", "enum", "trait", "impl", "interface", "method", "variable", "const", "let", "mut", "static", "public", "private", "protected", "return", "if", "else", "for", "while", "loop", "match", "switch", "case", "try", "catch", "error", "exception", "throw", "panic", "result", "option", "some", "none", "ok", "err", "string", "int", "float", "bool", "array", "vector", "list", "map", "hash", "set", "iterator", "closure", "lambda", "generic", "template", "macro", "derive", "annotation", "decorator", "import", "export", "module", "namespace", "package", "crate", "library", "framework", "api", "http", "json", "xml", "database", "sql", "nosql", "file", "io", "network", "thread", "process", "concurrent", "parallel", "sync", "mutex", "lock", "channel", "stream", "buffer", "memory", "heap", "stack", "garbage", "collection", "reference", "pointer", "ownership", "borrow", "lifetime", "unsafe", "safe", "performance", "optimization", "benchmark", "test", "unit", "integration", "debug", "log", "trace", "warning", "info", "config", "settings", "environment", "docker", "container", "deployment", "production", "development", "staging"
        ].iter().map(|s| s.to_string()).collect();
    ...
    fn extract_keywords(&self, normalized_query: &str) -> Vec<String> {
        normalized_query
            ...
            .filter(|word| {
                ...
                && (self.programming_keywords.contains(*word) || word.chars().any(|c| c.is_alphabetic()))
            })
    ...
    fn determine_intent(&self, normalized_query: &str, keywords: &[String]) -> String {
        ...
        if query_lower.contains("find") || query_lower.contains("search") || query_lower.contains("look for") {
            "search".to_string()
        } else if query_lower.contains("how") || query_lower.contains("what") || query_lower.contains("explain") {
            "explanation".to_string()
        } else if query_lower.contains("error") || query_lower.contains("fix") || query_lower.contains("debug") {
            "troubleshoot".to_string()
        } else if query_lower.contains("example") || query_lower.contains("show") || query_lower.contains("demonstrate") {
            "example".to_string()
        } else if keywords.iter().any(|k| self.programming_keywords.contains(k)) {
    ...
    fn classify_query_type(&self, normalized_query: &str, keywords: &[String]) -> Option<QueryType> {
        let query_lower = normalized_query.to_lowercase();
        
        if keywords.iter().any(|k| ["function", "fn", "method"].contains(&k.as_str())) {
            Some(QueryType::FunctionLookup)
        } else if query_lower.contains("error") || query_lower.contains("fix") || query_lower.contains("bug") {
...
mod tests {
    ...
    fn test_query_type_classification() {
        ...
        let test_cases = vec![
            ...
            ("how do I handle errors in Rust", QueryType::ConceptExplanation),
        ...
        for (query, expected_type) in test_cases {
            ...
            assert_eq!(query_type, Some(expected_type), "Failed for query: {}", query);
        }
    }
}