...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, query) in test_queries.iter().enumerate() {
        ...
        println!(
            "      - Query processing: {}ms",
            result.performance_metrics.query_processing_ms
        );
        println!(
            "      - Context retrieval: {}ms",
            result.performance_metrics.context_retrieval_ms
        );
        println!(
            "      - Result ranking: {}ms",
            result.performance_metrics.result_ranking_ms
        );
        println!(
            "      - Response generation: {}ms",
            result.performance_metrics.response_generation_ms
        );
        ...
        if duration.as_millis() <= 200 {
            ...
            println!(
                "   ⚠️  Performance target missed ({}ms > 200ms)",
                duration.as_millis()
            );
        }
    }
...
fn create_sample_nodes() -> Vec<CodeNode> {
    ...
    vec![
        CodeNode {
            ...
            span: None,
            content: Some("async fn read_file(path: &str) -> Result<String, std::io::Error> { tokio::fs::read_to_string(path).await }".to_string().into()),
            embedding: None,
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "write_file".to_string().into(),
            node_type: Some(NodeType::Function),
            language: Some(Language::Rust),
            span: None,
            content: Some("async fn write_file(path: &str, content: &str) -> Result<(), std::io::Error> { tokio::fs::write(path, content).await }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.4),
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "handle_error".to_string().into(),
            node_type: Some(NodeType::Function),
            language: Some(Language::Rust),
            span: None,
            content: Some("fn handle_error<T, E: std::fmt::Display>(result: Result<T, E>) -> Option<T> { match result { Ok(value) => Some(value), Err(e) => { eprintln!(\"Error: {}\", e); None } } }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.6),
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "fetch_data".to_string().into(),
            node_type: Some(NodeType::Function),
            language: Some(Language::Rust),
            span: None,
            content: Some("async fn fetch_data(url: &str) -> Result<String, reqwest::Error> { reqwest::get(url).await?.text().await }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.5),
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "process_input".to_string().into(),
            node_type: Some(NodeType::Function),
            language: Some(Language::Rust),
            span: None,
            content: Some("fn process_input(input: &str) -> String { input.trim().to_lowercase() }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.2),
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "connect_database".to_string().into(),
            node_type: Some(NodeType::Function),
            language: Some(Language::Rust),
            span: None,
            content: Some("async fn connect_database(url: &str) -> Result<Database, sqlx::Error> { sqlx::PgPool::connect(url).await }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.7),
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "UserService".to_string().into(),
            node_type: Some(NodeType::Struct),
            language: Some(Language::Rust),
            span: None,
            content: Some("struct UserService { db: Database, cache: Cache }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.3),
        ...
        CodeNode {
            id: Uuid::new_v4(),
            name: "validate_user".to_string().into(),
            node_type: Some(NodeType::Function),
            language: Some(Language::Rust),
            span: None,
            content: Some("async fn validate_user(user_id: u64) -> Result<bool, ValidationError> { /* validation logic */ Ok(true) }".to_string().into()),
            embedding: None,
            location: location.clone(),
            metadata: metadata.clone(),
            complexity: Some(0.5),
