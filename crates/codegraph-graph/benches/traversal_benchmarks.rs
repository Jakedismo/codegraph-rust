...
use codegraph_graph::{CodeEdge, CodeGraph, TraversalConfig};
...
async fn create_tree_graph(depth: usize, branching_factor: usize) -> (CodeGraph, NodeId, NodeId) {
    ...
    // Add root node
    let root_node = CodeNode::new(
        root,
        "root".to_string(),
        NodeType::Module,
        "/test/root.rs".to_string(),
    );
...
fn bench_bfs_traversal(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000, 2000].iter() {
        group.bench_with_input(BenchmarkId::new("dense_graph", size), size, |b, &size| {
...
fn bench_dfs_traversal(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000, 2000].iter() {
        group.bench_with_input(BenchmarkId::new("sparse_graph", size), size, |b, &size| {
            b.to_async(&rt).iter(|| async {
...
fn bench_shortest_path_algorithms(c: &mut Criterion) {
    ...
    for depth in [5, 10, 15, 20].iter() {
        ...
        // Dijkstra
        group.bench_with_input(BenchmarkId::new("dijkstra", depth), depth, |b, &depth| {
            b.to_async(&rt).iter(|| async {
                let (graph, root, leaf) = create_tree_graph(depth, 3).await;
        ...
        group.bench_with_input(BenchmarkId::new("astar", depth), depth, |b, &depth| {
            b.to_async(&rt).iter(|| async {
                let (graph, root, leaf) = create_tree_graph(depth, 3).await;
                ...
                let result = graph
                    .astar_shortest_path(root, leaf, heuristic)
                    .await
                    .unwrap();
                black_box(result);
            });
...
fn bench_graph_size_scaling(c: &mut Criterion) {
    ...
    for size in [500, 1000, 2000, 4000].iter() {
        // BFS scaling
        group.bench_with_input(BenchmarkId::new("bfs_scaling", size), size, |b, &size| {
            b.to_async(&rt).iter(|| async {
