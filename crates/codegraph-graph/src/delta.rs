...
impl GraphDeltaProcessor {
    ...
    pub async fn compute_delta(
        ...
        info!("Computing graph delta for {} -> {} nodes", old_nodes.len(), new_nodes.len());
        ...
        info!("Delta computation completed: {} operations in {}ms", 
              delta.operations.len(), stats.computation_duration_ms);
    ...
    async fn compute_delta_comprehensive(
        ...
        for (old_id, old_node) in old_nodes {
            if !new_nodes.contains_key(old_id) {
                ...
                debug!("Node removed: {} ({})", old_node.name.as_deref().unwrap_or("unnamed"), old_id);
            }
        }
        ...
        for (new_id, new_node) in new_nodes {
            match old_nodes.get(new_id) {
                None => {
                    ...
                    debug!("Node added: {} ({})", new_node.name.as_deref().unwrap_or("unnamed"), new_id);
                }
                Some(old_node) => {
                    // Check if node changed
                    if self.node_comparator.has_changed(old_node, new_node) {
                        operations.push(DeltaOperation::UpdateNode(new_id.clone(), (*new_node).clone()));
                        affected_nodes.insert(new_id.clone());
                        debug!("Node updated: {} ({})", new_node.name.as_deref().unwrap_or("unnamed"), new_id);
                    }
                }
            }
        }
    ...
    async fn compute_delta_optimized(
        ...
        new_nodes.par_iter().for_each(|(new_id, new_node)| {
            match old_nodes.get(new_id) {
                None => {
                    operations_map.insert(new_id.clone(), DeltaOperation::AddNode((*new_node).clone()));
        ...
        // Collect results
        operations.extend(operations_map.into_iter().map(|(_, op)| op));
        affected_nodes.extend(affected_nodes_set.into_iter().map(|(id, _)| id));
    ...
    pub async fn apply_delta(
        ...
        info!("Applying delta {} with {} operations", delta.id, delta.operations.len());
        ...
        info!("Delta application completed: {}/{} operations successful in {}ms",
              applied_operations, delta.operations.len(), duration.as_millis());
...
mod tests {
    ...
    async fn test_delta_computation() {
        ...
        let result = processor.compute_delta(
            &old_nodes,
            &new_nodes,
        ...
        assert!(matches!(result.delta.operations[0], DeltaOperation::UpdateNode(_, _)));
    }
    ...
    fn test_delta_size_estimation() {
        ...
        let old_nodes = vec![
            CodeNode { id: Some("1".to_string()), ..Default::default() },
            CodeNode { id: Some("2".to_string()), ..Default::default() },
        ];

        let new_nodes = vec![
            CodeNode { id: Some("1".to_string()), ..Default::default() },
            CodeNode { id: Some("3".to_string()), ..Default::default() },
        ];
