...
impl GitLikeVersionManager {
    ...
    pub async fn merge_branches(
        ...
        let merge_message =
            message.unwrap_or_else(|| format!("Merge branch '{}' into '{}'", source, target));

        self.storage
            .merge(source, target, author, merge_message)
            .await
    }
    ...
    pub async fn three_way_merge(
        ...
        Ok(MergeResult {
            success,
            conflicts,
            merged_version_id: if success {
                Some(VersionId::new_v4())
            } else {
                None
            },
    ...
    async fn get_commits_between(&self, from: VersionId, to: VersionId) -> Result<Vec<VersionId>> {
...
mod tests {
    ...
    fn test_topological_sort() {
        ...
        let positions: HashMap<VersionId, usize> =
