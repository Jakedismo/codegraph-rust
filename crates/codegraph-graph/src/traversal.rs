...
pub struct TraversalConfig {
    ...
    pub filter: Option<Box<dyn Fn(NodeId) -> bool + Send + Sync>>,
}
...
impl<'a> Stream for BfsIterator<'a> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        while let Some((current, depth)) = this.queue.pop_front() {
            ...
            let neighbors_future = async move {
                graph.get_neighbors(current).await
            };
...
impl<'a> Stream for DfsIterator<'a> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        while let Some((current, depth)) = this.stack.pop() {
            ...
            let neighbors_future = async move {
                this.graph.get_neighbors(current).await
            };
...
impl CodeGraph {
    ...
    pub async fn dijkstra_shortest_path(&self, start: NodeId, target: NodeId) -> Result<Option<Vec<NodeId>>> {
        ...
        while let Some(DijkstraNode { id: current, distance, path }) = heap.pop() {
    ...
    pub async fn astar_shortest_path<H>(&self, start: NodeId, target: NodeId, heuristic: H) -> Result<Option<Vec<NodeId>>>
    where
        H: Fn(NodeId, NodeId) -> f64 + Send + Sync,
    {
        ...
        while let Some(AStarNode { id: current, g_score, path, .. }) = heap.pop() {
            // If we've reached the target
            if current == target {
                return Ok(Some(path));
            }
    ...
    pub async fn detect_cycles(&self) -> Result<Vec<Vec<NodeId>>> {
        ...
        for node in all_nodes {
            if !visited.contains(&node) {
                if let Some(cycle) = self.dfs_detect_cycle(node, &mut visited, &mut rec_stack, &mut Vec::new()).await? {
                    cycles.push(cycle);
                }
            }
        }
    ...
    async fn dfs_detect_cycle(
        ...
        for neighbor in neighbors {
            if !visited.contains(&neighbor) {
                if let Some(cycle) = self.dfs_detect_cycle(neighbor, visited, rec_stack, path).await? {
    ...
    async fn get_all_node_ids(&self) -> Result<Vec<NodeId>> {
        ...
        for entry in self.node_cache.iter() {
        ...
        *index_counter += 1;
        ...
        for neighbor in neighbors {
            if !indices.contains_key(&neighbor) {
                self.tarjan_dfs(neighbor, index_counter, stack, indices, lowlinks, on_stack, components).await?;
