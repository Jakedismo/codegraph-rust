...
use codegraph_core::{
    CodeGraphError, CodeNode, NodeId, Result, 
    SnapshotId, TransactionId, VersionId, Transaction, Snapshot, Version, 
    NodeVersion, WriteAheadLogEntry, Checkpoint, WriteOperation,
    IsolationLevel, TransactionStatus, VersionedStore, TransactionManager, 
    WriteAheadLog, CrashRecovery, VersionDiff, ChangeType,
};
use rocksdb::{
    BlockBasedOptions, ColumnFamily, ColumnFamilyDescriptor, 
    DBWithThreadMode, MultiThreaded, Options, ReadOptions, 
    WriteBatch, WriteOptions, Cache, DBCompressionType, IteratorMode
};
...
impl VersionedRocksDbStorage {
    ...
    async fn initialize_counters(&self) -> Result<()> {
        ...
        let wal_iter = self.db.iterator_cf(&wal_cf, IteratorMode::End);
        if let Ok(Some((key, _))) = wal_iter.next() {
        ...
        let tx_iter = self.db.iterator_cf(&transactions_cf, IteratorMode::End);
        if let Ok(Some((key, _))) = tx_iter.next() {
            if let Ok(tx_id) = Uuid::parse_str(&String::from_utf8_lossy(&key)) {
                self.transaction_counter.store(tx_id.as_u128() as u64 + 1, Ordering::Relaxed);
            }
        }
    ...
    async fn store_content(&self, content: &[u8]) -> Result<String> {
        ...
        let existing = self.db.get_cf(&content_cf, &hash)
        ...
        self.db.put_cf(&content_cf, &hash, serialized)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
    ...
    fn get_commit_lock(&self, node_id: NodeId) -> Arc<Mutex<()>> {
        self.commit_locks.entry(node_id)
    ...
    async fn read_node_at_snapshot(
        ...
        let node: CodeNode = bincode::deserialize(&content)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
...
impl VersionedStore for VersionedRocksDbStorage {
    async fn create_snapshot(&mut self, transaction_id: TransactionId) -> Result<SnapshotId> {
        ...
        for (node_id, write_op) in &tx.write_set {
            match write_op {
                WriteOperation::Insert(id) | WriteOperation::Update { new_content_hash, .. } => {
                    let hash = match write_op {
                        WriteOperation::Update { new_content_hash, .. } => new_content_hash.clone(),
                        WriteOperation::Insert(_) => {
                            return Err(CodeGraphError::Transaction("Invalid insert operation".to_string()));
                        }
                        _ => unreachable!(),
                    };
        ...
        if let Some(parent_snapshot_id) = self.get_snapshot(tx.snapshot_id).await? {
            for (node_id, hash) in &parent_snapshot_id.node_versions {
                if !node_versions.contains_key(node_id) && 
                   !tx.write_set.contains_key(node_id) {
        ...
        let snapshot = Snapshot {
            ...
            transaction_id,
            node_versions,
            parent_snapshot: Some(tx.snapshot_id),
        ...
        self.db.put_cf(&snapshots_cf, snapshot_id.as_bytes(), serialized)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
    ...
    async fn create_version(
        ...
        self.db.put_cf(&versions_cf, version_id.as_bytes(), serialized)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
    ...
    async fn tag_version(&mut self, version_id: VersionId, tag: String) -> Result<()> {
        ...
        self.db.put_cf(&version_tags_cf, tag_key.as_bytes(), version_id.as_bytes())
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
    ...
    async fn get_version_by_tag(&self, tag: &str) -> Result<Option<Version>> {
        ...
        match self.db.get_cf(&version_tags_cf, tag_key.as_bytes()) {
            Ok(Some(version_id_bytes)) => {
                ...
                let version_id = Uuid::parse_str(&version_id_str)
                    .map_err(|e| CodeGraphError::Database(e.to_string()))?;
    ...
    async fn branch_from_version(
        ...
        let source_version = self.get_version(source_version).await?
    ...
    async fn compare_versions(
        ...
        let s1 = self.get_snapshot(v1.snapshot_id).await?
            .ok_or_else(|| CodeGraphError::Transaction("Snapshot 1 not found".to_string()))?;
        let s2 = self.get_snapshot(v2.snapshot_id).await?
        ...
        let all_nodes: HashSet<NodeId> = s1.node_versions.keys()
        ...
        for node_id in all_nodes {
            ...
            match (v1_hash, v2_hash) {
                (None, Some(hash)) => {
                    ...
                    node_changes.insert(node_id, codegraph_core::NodeDiff {
...
impl TransactionManager for VersionedRocksDbStorage {
    async fn begin_transaction(
        ...
        self.db.put_cf(&transactions_cf, transaction_id.as_bytes(), serialized)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
        
        let transaction_arc = Arc::new(RwLock::new(transaction));
        self.active_transactions.insert(transaction_id, transaction_arc);
    ...
    async fn commit_transaction(&mut self, transaction_id: TransactionId) -> Result<()> {
        ...
        
        {
            ...
            if tx.status != TransactionStatus::Active {
                return Err(CodeGraphError::Transaction("Transaction not active".to_string()));
            }
        ...
        if !self.validate_transaction(transaction_id).await? {
            ...
            return Err(CodeGraphError::Transaction("Transaction validation failed".to_string()));
        }
        
        let mut batch = WriteBatch::default();
        let node_locks: Vec<_> = {
            let tx = transaction.read();
            tx.write_set.keys().map(|node_id| (*node_id, self.get_commit_lock(*node_id))).collect()
        };
        ...
        match tx.isolation_level {
            IsolationLevel::Serializable => {
                for node_id in &tx.read_set {
                    for (_, other_tx) in self.active_transactions.iter() {
                        let other = other_tx.read();
                        if other.id != tx.id && 
...
impl WriteAheadLog for VersionedRocksDbStorage {
    async fn append_entry(&mut self, entry: WriteAheadLogEntry) -> Result<u64> {
        ...
        self.db.put_cf(&wal_cf, key.as_bytes(), serialized)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
    ...
    async fn get_entries_after(&self, sequence: u64) -> Result<Vec<WriteAheadLogEntry>> {
        ...
        let iter = self.db.iterator_cf_opt(&wal_cf, read_opts, IteratorMode::From(start_key.as_bytes(), rocksdb::Direction::Forward));
    ...
    async fn create_checkpoint(&mut self) -> Result<Checkpoint> {
        ...
        self.db.put_cf(&checkpoints_cf, checkpoint_id.as_bytes(), serialized)
            .map_err(|e| CodeGraphError::Database(e.to_string()))?;
