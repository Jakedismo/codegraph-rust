...
pub enum ContentRepairStrategy {
    RecreateFromSource { source_path: PathBuf },
    RecomputeFromNodes { node_ids: Vec<NodeId> },
    RestoreFromBackup { backup_timestamp: DateTime<Utc> },
    MarkAsCorrupted,
}
...
impl RecoveryManager {
    ...
    pub async fn start_background_integrity_checks(&self) -> Result<()> {
        ...
        task::spawn(async move {
            ...
            loop {
                ...
                let backup = std::path::PathBuf::from("/tmp/backup");
                let recovery_manager = RecoveryManager::new(&storage_path, &backup);
    ...
    pub async fn run_integrity_check(&self) -> Result<IntegrityReport> {
        ...
        if let Err(e) = self
            .check_snapshot_integrity(&mut issues, &mut orphaned_snapshots)
            .await
        {
        ...
        for (i, action) in actions.iter().enumerate() {
            ...
            match action_result {
                ...
                Err(_) => {
                    ...
                    return Err(CodeGraphError::Database(
                        "Recovery action timeout".to_string(),
                    ));
                }
            }
        }
    ...
    async fn execute_single_action(&self, action: &RecoveryAction) -> Result<()> {
        match action {
            RecoveryAction::ReplayTransaction {
                transaction_id,
                from_wal_sequence,
            } => {
                self.replay_transaction(*transaction_id, *from_wal_sequence)
                    .await
            }
            ...
            RecoveryAction::RepairContent {
                content_hash,
                repair_strategy,
            } => self.repair_content(content_hash, repair_strategy).await,
            RecoveryAction::RemoveOrphanedData {
                data_type,
                identifiers,
    ...
    async fn repair_content(
        &self,
        content_hash: &str,
        strategy: &ContentRepairStrategy,
    ) -> Result<()> {
        match strategy {
            ContentRepairStrategy::RecreateFromSource { source_path } => {
                // TODO: Read original source and recompute content
                let _content = fs::read(source_path).await.map_err(|e| {
                    CodeGraphError::Database(format!("Failed to read source: {}", e))
                })?;
            ...
            ContentRepairStrategy::RestoreFromBackup {
                backup_timestamp: _,
            } => {
                // TODO: Restore from backup
            }
            ContentRepairStrategy::MarkAsCorrupted => {
                self.quarantine_content(content_hash, QuarantineReason::CorruptedContent)
                    .await?;
            }
        }
    ...
    async fn quarantine_content(&self, content_hash: &str, reason: QuarantineReason) -> Result<()> {
        ...
        state
            .quarantined_data
            .insert(content_hash.to_string(), reason.clone());
...
mod tests {
    ...
    async fn test_recovery_plan_creation() {
        ...
        let report = IntegrityReport {
            timestamp: Utc::now(),
            issues: vec![IntegrityIssue::InvalidChecksum {
