...
use git2::{BranchType, DiffOptions, FileFavor, MergeOptions, Oid, Repository, RepositoryOpenFlags, Signature, StatusOptions};
...
impl GitRepository {
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        Ok(Self { path: path_ref.to_path_buf(), repo })
    }

    pub fn init<P: AsRef<Path>>(path: P) -> Result<Self> {
        let repo = Repository::init(path.as_ref())?;
        Ok(Self { path: path.as_ref().to_path_buf(), repo })
    }

    pub fn repository(&self) -> &Repository { &self.repo }
    pub fn workdir(&self) -> Option<&Path> { self.repo.workdir() }

    pub fn is_bare(&self) -> bool { self.repo.is_bare() }

    pub fn current_branch(&self) -> Result<Option<String>> {
        let head = match self.repo.head() {
            Ok(h) => h,
            Err(e) if e.code() == git2::ErrorCode::UnbornBranch || e.code() == git2::ErrorCode::NotFound => return Ok(None),
    ...
    pub fn list_branches(&self, include_remote: bool) -> Result<Vec<BranchInfo>> {
        ...
        for bt in [BranchType::Local, BranchType::Remote] {
            if bt == BranchType::Remote && !include_remote { continue; }
            let branches = self.repo.branches(Some(bt))?;
            for b in branches {
                let (branch, btype) = b?;
                let name = branch.name()?.unwrap_or("").to_string();
                let is_head = branch.is_head();
                let upstream = match branch.upstream() { Ok(u) => u.name()?.map(|s| s.to_string()), Err(_) => None };
                result.push(BranchInfo { name, is_head, is_remote: btype == BranchType::Remote, upstream });
            }
        }
        Ok(result)
    }
    ...
    pub fn status_summary(&self) -> Result<ChangeSummary> {
        if self.is_bare() { return Err(GitIntegrationError::BareRepository); }
        let mut opts = StatusOptions::new();
        opts.include_untracked(true).recurse_untracked_dirs(true).include_ignored(false);
        let statuses = self.repo.statuses(Some(&mut opts))?;
        let mut added=0; let mut deleted=0; let mut modified=0; let mut renamed=0; let mut files_changed=0;
        for s in statuses.iter() {
            let st = s.status();
            if st.is_wt_new() { added += 1; }
            if st.is_wt_deleted() { deleted += 1; }
            if st.is_wt_modified() { modified += 1; }
            if st.is_wt_renamed() { renamed += 1; }
            files_changed += 1;
        }
        Ok(ChangeSummary { added, deleted, modified, renamed, files_changed })
    }
    ...
    pub fn install_hook(&self, kind: HookKind, script: &str, overwrite: bool) -> Result<()> {
        ...
        if !hooks_path.exists() { fs::create_dir_all(&hooks_path)?; }
        let filename = match kind { HookKind::PreCommit => "pre-commit", HookKind::PostCommit => "post-commit" };
        let hook_path = hooks_path.join(filename);
        if hook_path.exists() && !overwrite { return Ok(()); }
    ...
    pub fn install_hooks(&self, opts: HookInstallOptions) -> Result<()> {
        ...
        if opts.pre_commit { self.install_hook(HookKind::PreCommit, PRE, opts.overwrite)?; }
        if opts.post_commit { self.install_hook(HookKind::PostCommit, POST, opts.overwrite)?; }
        Ok(())
    }

    pub fn merge_branches(&self, ours: &str, theirs: &str, strategy: MergeStrategy) -> Result<MergeOutcome> {
        let mut outcome = MergeOutcome { fast_forward: false, conflicts: 0, committed: false };
        let mut index = {
            let mut opts = MergeOptions::new();
            match strategy {
                MergeStrategy::Ours => { opts.file_favor(FileFavor::Ours); },
                MergeStrategy::Theirs => { opts.file_favor(FileFavor::Theirs); },
        ...
        if !index.has_conflicts() {
            ...
            let sig = self.repo.signature().or_else(|_| Signature::now("CodeGraph", "codegraph@example.com"))?;
            let head_commit = self.repo.head().ok().and_then(|h| h.target()).and_then(|oid| self.repo.find_commit(oid).ok());
            ...
            if let Some(head_commit) = head_commit {
                self.repo.commit(Some("HEAD"), &sig, &sig, &format!("Merge {} into {}", theirs, ours), &tree, &[&head_commit, &theirs_commit])?;
    ...
    fn try_fast_forward(&self, local_branch: &str, upstream_ref: &str) -> Result<bool> {
        ...
        let analysis = self.repo.merge_analysis(&[&self.repo.find_annotated_commit(upstream)?])?;
        if analysis.0.is_fast_forward() {
            lb_ref.set_target(upstream, "fast-forward")?;
            self.repo.set_head(&format!("refs/heads/{}", local_branch))?;
