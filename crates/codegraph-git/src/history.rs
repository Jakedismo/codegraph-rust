...
impl super::GitRepository {
    pub fn analyze_history(&self, opts: HistoryOptions) -> Result<HistoryInsights> {
        ...
        for (i, oid_res) in revwalk.enumerate() {
            ...
            if let Some(since) = opts.since_timestamp {
            ...
            *authors.entry((name, email)).or_insert(0) += 1;
            ...
            let _parent = match commit.parent(0) {
                Ok(p) => p,
                Err(_) => continue,
            };
                ...
                &mut |delta, _| {
                    if let Some(path) = delta.new_file().path().or_else(|| delta.old_file().path())
                    {
                        if let Some(path_str) = path.to_str() {
                            let entry =
                                file_churn.entry(path_str.to_string()).or_insert(FileChurn {
                                    additions: 0,
                                    deletions: 0,
                                });
            ...
            let agg = file_churn.entry("_aggregate".into()).or_insert(FileChurn {
                additions: 0,
                deletions: 0,
            });
        ...
        let mut authors_vec: Vec<AuthorStat> = authors
            .into_iter()
