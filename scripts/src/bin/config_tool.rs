...
enum Commands {
    ...
    Encrypt {
        ...
        #[arg(long, default_value = "config/secrets.enc")] output: PathBuf,
    },
    ...
    Decrypt {
        ...
        #[arg(long, default_value = "config/secrets.enc")] input: PathBuf,
    },
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Encrypt { key, input, output } => {
            let key_b64 = key.or_else(|| std::env::var("CONFIG_ENC_KEY").ok()).context("Provide --key or set CONFIG_ENC_KEY")?;
